"""
Streamlit UI: Cancer Prediction Studio

This app provides a simple interface to:
- Load input data (CSV or a built-in sample) with the expected features.
- Run classification (malignant vs benign) using a pretrained model.
- Run regression to predict a continuous feature (e.g., `radius_mean`).
- Rebuild models by calling the training pipeline.
- Visualize saved metrics and figures generated by the pipeline.

How to use quickly:
- Click "Rebuild Models (Train)" once to create artifacts and metrics.
- Use the sidebar to choose Classification or Regression.
- Load data (or enable the sample), then click "Predict".
- Scroll to the Model Report section to see metrics and visuals.
"""
...
def regression_feature_list(target: str) -> List[str]:
    """Return the feature list used for regression.

    The regressor predicts `target` from all other expected features,
    so we exclude the target column from inputs.
    """
...
# UI option: choose how numeric class IDs are displayed
class_id_scheme = st.sidebar.radio(
    "Class ID scheme",
    ["0 = Malignant, 1 = Benign (original)", "0 = Benign, 1 = Malignant (flipped)"],
    index=1,
    help="Controls numeric class IDs shown in prediction tables and downloads. Labels and probabilities remain unchanged."
)
one_is_malignant = class_id_scheme.endswith("Malignant (flipped)")
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This matches the naming used by the training pipeline and prevents
    subtle mismatches between CSV headers and expected feature names.
    """
    df = df.copy()
...
def ensure_expected_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Warns in the UI if any expected columns are missing.
    - Returns the dataframe with expected columns if all are present; otherwise
      returns the normalized dataframe unchanged so the caller can decide.
    """
...
def load_csv_from_upload(uploaded_file) -> Optional[pd.DataFrame]:
    """Read a CSV uploaded via Streamlit's file_uploader.

    Returns None and displays an error in Streamlit if reading fails.
    """
    try:
...
def read_metrics() -> Dict:
    """Load metrics saved by the training pipeline from `reports/metrics.json`.
...
def predict_classification(df: pd.DataFrame, one_is_malignant_ui: bool = False) -> Optional[pd.DataFrame]:
    """Run classification predictions for the provided features.

    - Loads the best classifier artifact from `artifacts/classifier_best.joblib`.
    - Predicts the class for each row and appends:
      - `prediction` (0=Malignant, 1=Benign)
      - `label` ("Malignant"/"Benign")
      - `prob_benign` and `prob_malignant` (probabilities, if available)
    - Returns None and shows an error if the artifact is missing or prediction fails.
    """
    ...
    try:
        ...
        # Try to get probabilities; fall back to scaled decision_function
        prob_benign, prob_malignant = None, None
        try:
            if hasattr(model, "predict_proba"):
                proba = model.predict_proba(X)
                prob_benign = proba[:, 1]
                prob_malignant = proba[:, 0]
            else:
                scores = model.decision_function(X)
                # scale scores to 0..1 as a proxy for probability of benign
                s_min, s_max = float(scores.min()), float(scores.max())
                scaled = (scores - s_min) / (s_max - s_min + 1e-9)
                prob_benign = scaled
                prob_malignant = 1.0 - scaled
        except Exception:
            pass

        out = df.copy()
        # Optionally flip the numeric class IDs for UI purposes
        out["prediction"] = [1 if p == 0 else 0 for p in preds] if one_is_malignant_ui else preds
        out["label"] = ["Benign" if p == 1 else "Malignant" for p in preds]
        if prob_benign is not None:
            out["prob_benign"] = prob_benign
        if prob_malignant is not None:
            out["prob_malignant"] = prob_malignant
        return out
    except Exception as e:
...
def predict_regression(df: pd.DataFrame, target: str) -> Optional[pd.DataFrame]:
    """Run regression predictions for the provided features.

    - Loads the best regressor artifact for the requested target.
    - Predicts a continuous value and appends a `prediction` column.
    - Returns None and shows an error if the artifact is missing or prediction fails.
    """
...
def feature_template() -> pd.DataFrame:
    """Provide a single-row dataframe with reasonable default values.

    If a dataset snapshot exists (created by the training pipeline), use its first row
    to prefill defaults; otherwise return zeros for all features.
    """
...
if manual_mode == "Table":
    ...
    with mcol1:
        if st.button("Predict (Manual) - Classification"):
            df_out = predict_classification(manual_df, one_is_malignant)
        ...
        if df_input is None:
            ...
            with st.spinner("Running predictions..."):
                if task == "Classification":
                    df_out = predict_classification(df_input, one_is_malignant)
                else:
...
st.caption(
    "Classification class IDs shown: 1 = Malignant, 0 = Benign" if one_is_malignant else
    "Classification class IDs shown: 0 = Malignant, 1 = Benign"
)
