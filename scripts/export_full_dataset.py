"""
Export the full breast cancer dataset to `data.csv` with standardized names.

This script mirrors the feature renaming used by the training pipeline and
includes the `target` column. It can be used by notebooks or other tools
expecting a simple CSV in the project root.
"""
...
# Standardize column names like elsewhere
RENAMED = {
...
full_df = pd.concat([features_df, labels], axis=1)

Path(".").mkdir(exist_ok=True)
full_df.to_csv("data.csv", index=False)
print("Saved data.csv:", full_df.shape)
